...
- JDK：使用 Java 17；本仓库 `sourceCompatibility`、`targetCompatibility`、`jvmTarget` 和 Java toolchain 都指向 17。
- Gradle：使用仓库自带 Wrapper，当前版本为 `8.13`，定义在 `gradle/wrapper/gradle-wrapper.properties`。
- 构建命令：统一使用 `./gradlew`，不要假设本机全局 Gradle 版本可替代 Wrapper。
- 默认验证命令：`./gradlew :app:assembleDebug`
- 未经用户明确要求，不要擅自升级 JDK、Gradle 或 Android Gradle Plugin；若构建失败，先检查环境版本与仓库配置是否一致，再判断是否为代码问题。
...
5. 若本次任务涉及代码、资源或 Manifest 修改，默认运行一次 `./gradlew :app:assembleDebug`，确认不存在语法错误、资源引用错误或其他明显编译问题。
6. 运行必要验证。
7. 在最终说明中写清修改点、验证结果和剩余风险。

## 16. 验证要求

完成有意义的改动后，默认至少执行或评估以下验证：

1. 若涉及代码、资源或 Manifest 修改，至少执行一次构建应用：`./gradlew :app:assembleDebug`
